...
let zipVectorWith f (v:Vector4) (w:Vector4) =
    if v.W = w.W
        then Vector4 (f v.X w.X, f v.Y w.Y, f v.Z w.Z, v.W)
        else raise (System.ArgumentException "The two vector's w components doesn't match")
...
let toChildSpace n = 
    ...
    let f b = if b then -2.f else -1.f
